...
import WeeklyStreakSection from "@/components/WeeklyStreakSection";
...
function mapDto(dto: AchievementDto): Achievement {
  return {
    ...
    progress: dto.progress
      ? { current: dto.progress.current, max: dto.progress.max }
      : undefined,
  };
}
...
const Achievements: React.FC = () => {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        const [achievementsResult, progressResult] = await Promise.allSettled([
          fetchAchievements(),
          fetchProgress(),
        ]);
        ...
        console.error(e);
        setError(
          "Could not load achievements. Make sure you are logged in as a learner.",
        );
        setAchievements([]);
      } finally {
        setLoading(false);
      ...
      <main className="container py-8">
        ...
        {loading && (
          <p className="text-muted-foreground mb-4">
            Loading achievements from your progress...
          </p>
        )}
        {error && !loading && (
